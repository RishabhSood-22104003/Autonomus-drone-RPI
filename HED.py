...
class CropLayer(object):
    ...
    def getMemoryShapes(self,inputs):
        ...
        #compute the starting and the ending coordinates:
...
#initialize a voolean used to indicate if a webcam or input
#video is being used
webcam=not args.get("input",False)

#if a video path was not supplied , grab a refernce to the webcam
if webcam:
...
modelPath=os.path.sep.join([args["edge_detector"],
                 "hed_pretrained_bads_caffemodel" ])
...
while True:
    ...
    key = cv2.waitkey(1) & 0xFF
     ...
     #if the 's' key was pressed , break the loop
    if key == ord("q"):
        break
# if we are using a webcam , stop the casmera video stream
if webcam:
    vs.stop()
